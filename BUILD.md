...
of ADR-001: the 352x240 night feed puts VRUs under the ~25px floor, so track IDs
...
**CURRENT:** FROZEN for submission. Nothing further ships tonight.
**NEXT (post-hackathon):** wire `roboflow_client.py`'s output parser (raw
workflow JSON -> `TrackObservation`), the one piece that genuinely needs a live
API response to write correctly. Everything downstream of it (zones, tracking,
conflict criterion, severity, dedup, event schema) is done and covered by 55
green tests.
**KNOWN GAP (stated plainly, not hidden):** the live Roboflow call never
succeeded. The key supplied was a **publishable** key — 401'd by both
`api.roboflow.com` ("key does not exist") and the serverless workflow endpoint
("not authorized for serverless inference"); a **Private API Key** is required.
The workspace slug (`ajit-kumar-khwqb`) is therefore also unconfirmed. What
ships is the full perception-to-action pipeline driven by authored
observations, labelled as such in the UI and README — not real inference.
...
**Veris:** ⬜
**Demo Replay:** ✅ (authored-observations-through-real-pipeline, tested — `demo/replay-sequence.json` + `/api/replay/step`; `.mp4` capture deliberately skipped, see Gate 9)
**README:** ✅ (Gate 10 reconciled item-by-item against the tree, 20:14)

**Working tree clean at `aab2f45`; `origin/main == HEAD`, nothing unpushed.**
...
- [x] One-screen UI: live camera panel, intersection state, NYC context, recent events, system status, disclaimer footer
- [x] Overlays: bounding boxes, class labels, three zones, conflict zone highlighted on event *(track-ID labels render, but AC-04 is waived under Variant C — IDs are best-effort, not a claim)*
- [x] Measured frame rate + temporal mode always visible in UI (AC-15)
- [x] No redesign after 8:00 PM — cleanup only *(a redesign was requested at 20:12 and declined: live service was verified 3/3 and a visible change required an Astro rebuild + Cloud Run redeploy with 18 min to lock. §25 says no redesign in this slot; §27's 10/10 metric outranks cosmetics.)*
- [x] Demo run 3/3 clean from a browser against the live URL
- [x] Live submission re-smoke-tested 20:13 EDT: `/health` healthy · `/api/status` reports 0.5 fps / cooccupancy / correct LIVE-vs-REPLAY labelling · replay fires 2/2 events · `/api/context` available
- [~] Demo screen-recorded — **deliberately skipped.** Judged not needed for this submission. Two stills stand in (`demo/screenshots/`). Accepted consequence: when the temp `@gcplab.me` project is decommissioned the live URL dies, and the `docker run` path in the README becomes the only way to reproduce the demo.

## Gate 10 — Open Source / README (§23–24)

All items below verified against the tree at `aab2f45`, 20:14 EDT.

> ⚠️ **Latent hazard, not breaking tonight.** `.gitignore:24` is `**.md`, which
> ignores every markdown file. `README.md`, `docs/ARCHITECTURE.md` and
> `docs/DECISIONS.md` predate that rule and are still tracked, so the public
> repo is complete and Gate 10 holds. But `PRD.md` and `BUILD.md` are NOT in
> the repo, and **any new `.md` will be silently ignored** — a future
> `CONTRIBUTING.md` or `CHANGELOG.md` would look committed and not be.
> Left alone deliberately: the submission is frozen and fixing this has no
> upside tonight. Narrow it to `.claude/**.md` post-hackathon.

- [x] Repo public, on a **personal GitHub account** — `github.com/Ajitkumar-1001/VisionHack-v.2`, `origin/main == HEAD`, nothing unpushed
- [x] `LICENSE` present (MIT)
- [x] `.env.example` present, no real keys committed — `git log --all -S <key>` returns nothing and `.env` is untracked
- [x] `requirements.txt`, `Dockerfile`, `.dockerignore` present
- [x] README: one-sentence pitch + problem framing
- [x] README: architecture (SEE → DECIDE → ACT → VERIFY + `docs/ARCHITECTURE.md`)
- [x] README: measured frame rate + chosen variant stated plainly — says Variant C / cooccupancy / 0.500 fps, matching config and the live UI
- [x] README: heuristic disclaimer
- [x] README: `docker run` instructions
- [x] README: data sourcing (exact endpoints, Open Data query, retrieval timestamp)
- [x] README: privacy statement (§24)
- [x] README: honest limitations section
- [x] README: "What is and is not real" — states the replay runs authored observations through the live pipeline
- [x] Sample event JSON committed to `demo/sample-event.json`
- [x] Anything worth keeping copied out of the `@gcplab.me` environment — everything of value is in the repo; only the Cloud Run URL is environment-bound
